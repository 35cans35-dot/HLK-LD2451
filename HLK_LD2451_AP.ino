...
<main>
  ...
  <section>
    ...
    <label><input type="checkbox" id="phoneBeep"> Phone beep</label>
    <label>Tone Hz</label>
...
function connectWS(){
  ...
  ws.onmessage = (ev)=>{
    ...
    if(data.type === 'telemetry'){
      ...
      updateStats(data);
      drawRadar(data);
      if(data.warn_active && !lastWarnActive){
        playPhoneBeep(160, 1400);
      }
      lastWarnActive = data.warn_active;
...
function playPhoneBeep(durationMs=120, freq=1200){
  if(!qs('phoneBeep').checked) return;
...
function applyConfig(c){
  ...
  const storedPhoneBeep = localStorage.getItem('phoneBeep');
  qs('phoneBeep').checked = storedPhoneBeep === null ? true : storedPhoneBeep === '1';
  updateLabels();
}
...
qs('btnBeep').onclick = ()=>{
  ensureAudio();
  playPhoneBeep(140, 1500);
  fetch('/api/beep');
};
qs('btnMute').onclick = ()=> fetch('/api/mute');
qs('phoneBeep').onchange = ()=>{
  localStorage.setItem('phoneBeep', qs('phoneBeep').checked ? '1' : '0');
  ensureAudio();
};
